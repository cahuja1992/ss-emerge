...
class SEEDDataset(Dataset):
    ...
    def __init__(self, data_path, labels_path, sfreq=200, is_train=True, 
        ...
        """
        Args:
            data_path (str): Path to the .npy file containing EEG data (e.g., 'x_train_SEED.npy').
                             Expected shape after loading: (num_samples, 1, num_channels, time_points)
                             or (num_samples, num_channels, time_points).
            labels_path (str): Path to the .npy file containing labels (e.g., 'y_train_SEED.npy').
        ...
        if self.labels.ndim > 1:
            self.labels = self.labels.reshape(-1) # Flatten if (num_samples, 1) or (num_samples, num_trials)
        ...
        # Pre-calculate DE features for all samples to speed up __getitem__
        print(f"Loading and processing {len(self.eeg_data)} samples for DE features. This may take a moment...")
        processed_features = []
        for i, sample_data in enumerate(self.eeg_data):
            ...
            if (i + 1) % 1000 == 0:
                print(f"  Processed {i+1}/{len(self.eeg_data)} samples.")
        self.processed_features = np.array(processed_features, dtype=np.float32)
        print("DE feature processing complete.")
        
    def __len__(self):
        return len(self.eeg_data) # Or len(self.processed_features)
